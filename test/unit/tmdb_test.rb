...
class TmdbTest < Test::Unit::TestCase
  ...
  test "should perform Movie.getInfo API call and return array of results" do
    movies = Tmdb.api_call('movie',{:id => 187} )
  ...
  test "should perform Movie.imdbLookup API call and return array of results" do
    movies = Tmdb.api_call('movie', {:id => "tt0401792"})
    assert_kind_of Array, movies
    assert movies.length == 1
    movies.each do |movie|
      assert_kind_of Hash, movie
  ...
  test "should perform person API call and return array of results" do
    people = Tmdb.api_call('person', {:id => 287})
    assert_kind_of Hash, people
      %w(homepage id name).each do |item|
        assert_not_nil people[item]
      end
  end
